...
Use this boundary to screen and project untrusted tool results before the next model call. The full output remains in eve's durable event stream. Read [Prompt injection mitigation](/docs/guides/prompt-injection) before returning web pages, messages, documents, or other attacker-controlled content.
...
- [Prompt injection mitigation](/docs/guides/prompt-injection): mediate untrusted input and limit its consequences
