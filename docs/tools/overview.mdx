...
After delegation, the initiating parent turn and each task-triggered parent turn include
runtime-authored state for tasks started by the same parent turn. The model is instructed to keep
the initiating acknowledgement and related intermediate results silent while that state contains
pending tasks. Once every related task is terminal, the state includes their outputs so the model
can combine the useful results.
