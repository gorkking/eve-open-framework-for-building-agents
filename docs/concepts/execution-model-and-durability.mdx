...
eve's H3 router hosts the HTTP routes and workflow entrypoints, while CrossWS provides the Node and WebSocket transport. Neither supplies the workflow state store or the sandbox runtime. Those are separate adapters: Workflow uses the active world implementation, and Sandbox uses the backend from `agent/sandbox` or `defaultBackend()`.
