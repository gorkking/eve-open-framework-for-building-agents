...
Use an agent workspace when one Vercel project should deploy several peer eve agents together. The root owns the Vercel link, deployment, shared environment, and public route prefixes, while member packages can still own dependencies and build scripts. For example, a support and research suite can deploy from one root and serve its agents at `/eve/agents/support/eve/v1/*` and `/eve/agents/research/eve/v1/*`.
