...
The browser conversation lives durably on the server. Persist both the rendered event log and the `session` cursor to pick it back up after a reload. For storage options, write ordering, named chat threads, and hook-based archival, see [Chat persistence](../chat-persistence).

A local-storage example uses the same restoration contract:
...
- [Chat persistence](../chat-persistence): store named chats, session cursors, and ordered events
