...
An app without Web Chat uses `placeholderAuth()`. In production, it returns `401` for browser requests until you replace it with a real `AuthFn`. Delete the authored channel file and eve falls back to `[vercelOidc(), localDev()]`, which also does not admit browser users in production.
