...
`GET()` and `WS()` can use the same path. eve sends WebSocket upgrade requests
to `WS()` and ordinary GET requests to `GET()`. An upgrade request that matches
only an HTTP route is rejected instead of opening a connection without hooks.
