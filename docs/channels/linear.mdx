...
Select **Linear Agent** in the component checklist. **Linear tools** is also selected by default if you want the agent to search and update Linear through MCP. Run `eve add channel/linear-agent` instead to install only the channel directly.
