...
stream. When the agent requests input, pass the reported responses to
`agent_update`. Cancellation is cooperative; poll the invocation until it
reports `cancelled` or another terminal state.
