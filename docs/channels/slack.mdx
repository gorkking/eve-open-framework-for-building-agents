...
When Slack includes a preview for a link pasted into an accepted mention, DM, or channel message, eve adds up to five previews to model context. Each preview is capped at 2,000 characters and labeled as untrusted quoted content rather than instructions. This uses only the unfurl Slack already included in the inbound event and does not fetch the linked message or require additional scopes.
