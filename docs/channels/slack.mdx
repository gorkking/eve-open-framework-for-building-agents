...
export default slackChannel({
  ...
  threadReplies: {
    ...
    async onReply(message, ctx) {
      if (ctx.isBotMentioned) return { respond: true, state: null };
      ...
      return { respond: output.respond, state: null };
    },
  },
});
```
