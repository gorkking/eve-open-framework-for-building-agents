...
Start a session without an inbound message through `ctx.to(slack, target).send(message, { auth })` from another channel, or `to(slack, target).send(message, { auth })` from a schedule `run` handler. The proactive target shape is `{ channelId, teamId?, threadTs?, initialMessage? }`. Set `teamId` to the app installation workspace when a function-form bot token must select an installation.
