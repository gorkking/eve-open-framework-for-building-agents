...
Message hooks return `{ auth }` to dispatch, `null` to drop, or `{ auth, context }` to inject background into history. Add `title` to set the Agent Runs title without changing the model message. When `title` is omitted, direct messages use `Run` so private content does not appear in the run list; other Slack messages keep their message-derived titles.
