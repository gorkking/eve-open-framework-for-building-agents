...
- for `eve dev`, whether the target is local or remote and whether the UI is headless or interactive; and
- a random session identifier generated for that CLI invocation.

Telemetry does not include command arguments, agent files, prompts, URLs, request headers, error messages, arbitrary error properties, environment variables, file paths, or file contents.
...
The saved setting is stored in the platform user configuration directory:
