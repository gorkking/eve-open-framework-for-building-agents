...
The report is attempted only after a successful build. It records total elapsed time, completed phase timings, and final regular-file totals for file count, raw bytes, and the sum of each file compressed with gzip. It also aggregates the Rolldown builds eve starts directly, including invocation duration, total module occurrences, and unique modules by authored-module and workflow category. `totalInvocationDurationMs` is the sum of individual invocation durations, so concurrent builds can make it greater than wall-clock time.

The `nitro` section describes the final hosted server graph. It reports Nitro graph invocations, chunks, rendered module occurrences, unique modules, and rendered length. Package and eve-subsystem groups are sorted by rendered length, and `largestModules` contains the 25 largest normalized module IDs. Paths use `eve:`, `npm:`, `app:`, and `virtual:` prefixes instead of machine-specific roots. A module can occur in more than one Nitro graph invocation, so rendered lengths and occurrences can exceed their unique-module counts. For Vercel output the report also includes a size subtotal for every real `.func` directory, so app and flow bundles can be compared separately. The profile path resolves from the app root and should be outside the published output directory; profile collection does not add a file to the deployment. If collection or writing fails, eve emits a warning but keeps the completed build successful.
