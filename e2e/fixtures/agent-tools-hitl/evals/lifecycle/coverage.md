...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 · Store foundation             | `pnpm --filter eve exec vitest run --config vitest.unit.config.ts src/harness/hitl/obligations.test.ts`                                                                                                                                                                                               | Legacy batches migrate and reload without loss; uncut authorization, route, and deferred-input keys remain owned by their current paths; ledger invariants reject invalid references. |
| 2 · Interpreter extraction       | `pnpm --filter eve exec vitest run --config vitest.unit.config.ts src/harness/hitl/obligations.test.ts src/harness/hitl/interpret.test.ts src/harness/hitl/execute.test.ts src/harness/hitl/delivery.test.ts`; one literal state/input/decision case per migrated transition row | Legacy comparisons pass; real admission identity reaches the interpreter; approval candidates require adjudication; persisted effect plans resume at the first incomplete effect.     |
...
| owner.approval.compound.settle-then-run-siblings-open   | Planned | `hitl/interpret.test.ts`                                                                                                            | Compound message with an open sibling                         |
...
| owner.approval.response.reject-policy-failed            | Planned | `hitl/interpret.test.ts`                                                                                                            | Policy throw/timeout result                                   |
| owner.approval.response.reject-invalid                  | Planned | `hitl/interpret.test.ts`                                                                                                            | Invalid-value result                                          |
...
| owner.batch.park.dedupe-open-intent                     | Planned | `hitl/interpret.test.ts` + park-side intent index                                                                                   | Open-approval intent-key check at park                        |
...
| owner.batch.message.dismiss-question-only               | Planned | `hitl/interpret.test.ts`                                                                                                            | Mixed-group supersession                                      |
