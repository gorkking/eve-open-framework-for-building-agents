...
import { createBundleReportComparison } from "./bundle-report-compare.mjs";
...
function renderMetadataSection(report) {
  ...
  lines.push(
    `- Target: \`Vercel Build Output API v${report.buildMetadata?.outputVersion ?? "unknown"}\``,
  );
  lines.push(
    `- Framework: \`${report.buildMetadata?.frameworkName ?? "unknown"}@${report.buildMetadata?.frameworkVersion ?? "unknown"}\``,
  );
  lines.push(
    `- Output directory: \`${normalizePath(relative(process.cwd(), report.outputDirectory))}\``,
  );
  lines.push(`- Report generated: ${formatDate(report.generatedAt)}`);
  lines.push(
...
/**
 * Collects a bundle report from one Vercel build output tree.
 */
export async function collectBundleReport(options) {
  ...
  const [config, functionEntries, packageReports, staticFiles] = await Promise.all([
  ...
  return {
    ...
    buildMetadata: {
      frameworkName: typeof config?.framework?.slug === "string" ? config.framework.slug : null,
      frameworkVersion:
        typeof config?.framework?.version === "string" ? config.framework.version : null,
      outputVersion: typeof config?.version === "number" ? config.version : null,
    },
...
/**
 * Compares one bundle report against a baseline snapshot.
 */
export function compareBundleReports(report, baselineReport, options) {
  return createBundleReportComparison(report, baselineReport, options);
}

/**
 * Renders one Markdown report suitable for a GitHub PR comment or job summary.
 */
export function renderBundleReportMarkdown(report) {
...
function printUsage() {
  process.stdout.write(
    [
      "Usage: node ./scripts/bundle-report.mjs --app <path> [options]",
      "",
      "Options:",
...
async function main() {
  ...
  const outputReport =
    baselineReport === null
      ...
      : {
          ...report,
          comparison: compareBundleReports(report, baselineReport, {
            baselineLabel: argumentsResult.baselineLabel,
          }),
          sizeBudgetAcknowledged: argumentsResult.sizeBudgetAcknowledged,
        };
  const markdown = renderBundleReportMarkdown(outputReport);
