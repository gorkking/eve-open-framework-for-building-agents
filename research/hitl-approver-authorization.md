...
```text
Slack responder clicks Approve
...
approval: {
  ...
  async authorizeResponse({ request, responder, session, auth }) {
    ...
    return githubApprover.isEligible({ identity, request });
  },
}
```

Function shorthand remains the request-time-only form. The object form adds an
opt-in response authorizer. The callback receives the stable original request
(`toolName`, typed/raw tool input, `callId`, `requestId`), verified responder,
...
The default interaction is **Approve / Cancel**. Approve invokes the authorizer;
Cancel is ordinary authenticated conversation flow control and does not. An
approval candidate may be allowed, rejected with an authored `safeReason`, parked
...
1. **Verified responder only.** A response authorizer receives the identity
...
3. **Candidate binding.** An approval challenge is bound to `{ requestId,
...
- leave shared controls actionable while any approval candidate is nonterminal;
- replace/disable the card only after accepted durable settlement;
- render **Approve / Cancel**, with Cancel using normal interaction auth.
...
approved candidate to settle wins atomically; every later completion is stale and
...
The default approval interaction is **Approve / Cancel**. Approve is the only
...
Cancel and approved candidates share one atomic terminal-settlement race. If
Cancel settles first, the action does not run, every active approval candidate is
invalidated, and later authorization callbacks are stale and harmless. If an
approved candidate settles first, a later Cancel is stale.

Active validation never blocks Cancel, and Cancel cannot revoke an approval after
...
Change the approval negative approval option ID directly from `"deny"` to `"cancel"` while retaining `"approve"`. Render and
persist Approve/Cancel consistently across the protocol, history, events, clients,
