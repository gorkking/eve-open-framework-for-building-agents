...
describe("model catalog loaders", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("fetches the catalog once per runtime loader", async () => {
    mockCatalogFetch();
    const loader = createRuntimeModelCatalogLoader();
    await Promise.all([
      loader.getModelLimits("anthropic/claude-opus-4.7"),
      loader.getModelLimits("openai/gpt-5.4"),
    ]);
