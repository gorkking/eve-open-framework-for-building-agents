...
} from "#internal/host/dev-authored-rebuild-coordinator.js";
import { DrainedDevServer } from "#internal/host/drained-dev-server.js";
import type { DevelopmentRunnerFactory } from "#internal/host/dev-runner.js";
...
function createHost(
  ...
  return {
    ...
    workspace: {
      ...
      hostBuildDir: `/tmp/eve-test/.eve/dev-hosts/${id}/host`,
      hostOutputDir: `/tmp/eve-test/.eve/dev-hosts/${id}/output`,
...
async function createCoordinatorWithServer() {
  const devServer = new DrainedDevServer({ error: () => undefined }, createRunner);
...
describe("transactional authored rebuild coordinator", () => {
  ...
  it("discards the candidate and keeps prior state when the swap itself fails", async () => {
    ...
    const devServer = new DrainedDevServer({ error: () => undefined }, failingFactory);
    mocks.computeDevelopmentHostFingerprint.mockResolvedValueOnce("host-1");
    const coordinator = await createDevelopmentAuthoredRebuildCoordinator({
      devServer,
      initialHost: createHost("initial", "run-1"),
