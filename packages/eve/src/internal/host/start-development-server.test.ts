...
import type { DevelopmentServerHandle, DevelopmentServerOptions } from "#internal/host/types.js";
  ...
  return {
    ...
    prepareDevelopmentApplicationHost: vi.fn(async () => ({
      ...
      workspace: {
        ...
        hostBuildDir: "/tmp/eve-test/.eve/dev-hosts/test/host",
        hostOutputDir: "/tmp/eve-test/.eve/dev-hosts/test/output",
    ...
    resolveApplicationCompiledArtifactsSource: vi.fn(() => ({
...
vi.mock("./drained-dev-server.js", () => ({
  DrainedDevServer: mocks.createDevServer,
}));
...
> {
  const { createDevelopmentServer } = await import("#internal/host/start-development-server.js");
...
describe("createDevelopmentServer", () => {
  ...
  it("pins local workflow queue callbacks to the active dev server URL", async () => {
    ...
    expect(mocks.buildDevelopmentHostCandidate).toHaveBeenCalledWith({
      host: expect.objectContaining({ appRoot: "/tmp/eve-test" }),
    });
    expect(mocks.devServer.replaceWorker).toHaveBeenCalledWith({
  ...
  it("reuses the active server recorded for the same app root when requested", async () => {
    ...
    expect(mocks.buildDevelopmentHostCandidate).toHaveBeenCalledOnce();
