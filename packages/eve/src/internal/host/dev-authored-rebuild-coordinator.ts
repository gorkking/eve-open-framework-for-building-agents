...
import { computeDevelopmentHostFingerprint } from "#internal/host/dev-host-fingerprint.js";
...
class TransactionalDevelopmentAuthoredRebuildCoordinator implements DevelopmentAuthoredRebuildCoordinator {
  ...
  async #commitStructuralHost(input: {
    ...
    const payload = await buildDevelopmentHostCandidate({ host: committedHost });
...
function startSandboxPrewarmAfterCommit(
  ...
  const artifactsConfig = createDevelopmentApplicationArtifactsConfig({
  ...
  startDevelopmentSandboxPrewarmInBackground({
    appRoot: host.appRoot,
    compiledArtifactsSource: resolveApplicationCompiledArtifactsSource(artifactsConfig),
