...
import { DrainedDevServer } from "#internal/host/drained-dev-server.js";
import { createDevelopmentApplicationArtifactsConfig } from "#internal/host/artifacts-config.js";
import type { AuthoredSourceWatcherHandle } from "#internal/host/dev-authored-source-watcher.js";
import { prepareDevelopmentApplicationHost } from "#internal/host/prepare-application-host.js";
import { buildDevelopmentHostCandidate } from "#internal/host/dev-host-candidate.js";
import { removeDevelopmentHostWorkspace } from "#internal/host/dev-host-workspace.js";
import { createDevelopmentAuthoredRebuildCoordinator } from "#internal/host/dev-authored-rebuild-coordinator.js";
import {
...
import { DevelopmentServerState } from "#internal/host/dev-server-state.js";
...
} from "#internal/host/dev-server-url.js";
...
type OwnedDevelopmentServer = DrainedDevServer;
...
function addDevelopmentControlHandler(input: {
  readonly appRoot: string;
  readonly devServer: DrainedDevServer;
...
async function closeDevelopmentServerResources(input: {
  ...
  readonly devServer: OwnedDevelopmentServer | undefined;
...
async function listenForDevelopmentServer(input: {
  readonly devServer: OwnedDevelopmentServer;
  ...
  throw new Error(
    `Failed to start the development server after ${ports.length} attempts. Tried ports ${ports.join(", ")}.`,
    {
      cause: lastError,
    },
  );
...
async function startOwnedDevelopmentServer(
  ...
  let devServer: OwnedDevelopmentServer | undefined;
  ...
  try {
    ...
    const compiledArtifactsSource = resolveApplicationCompiledArtifactsSource(
    ...
    devServer = new DrainedDevServer();
    ...
    const hostname = options.host ?? DEFAULT_DEVELOPMENT_SERVER_HOST;
    ...
    if (!server.url) {
      throw new Error("Development server did not expose a URL.");
    }
    ...
    await devBootPhase(
      "building dev bundle",
      async () => {
        const payload = await buildDevelopmentHostCandidate({ host: preparedHost });
    ...
    authoredSourceWatcher = await devBootPhase(
      ...
      async () => {
        const { startAuthoredSourceWatcher } =
          await import("#internal/host/dev-authored-source-watcher.js");
 ...
 * owned development server or attach to a running owner, and `close()` to tear down a
 ...
 *
 * Authored schedules are never registered with the production cron scheduler in dev
...
export function createDevelopmentServer(
  ...
  return {
    start(): Promise<DevelopmentServerHandle> {
      ...
      startPromise = startOwnedDevelopmentServer(rootDir, options).then(({ handle, close }) => {
