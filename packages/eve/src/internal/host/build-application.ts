...
import { copyHostMiddlewareFunctions } from "#internal/host/copy-host-middleware.js";
import { prepareProductionApplicationHost } from "#internal/host/prepare-application-host.js";
import { runVercelBuildPrewarm } from "#internal/host/vercel-build-prewarm.js";
...
/**
 * Builds the production output for an eve application.
 */
export async function buildApplication(
  ...
  // so a mistaken run fails with a clear redirect instead of a half-configured path.
...
async function buildApplicationInWorkspace(
  ...
  const servicePrefix = isVercelBuild
    ...
    : undefined;
  const isServiceOutput = servicePrefix !== undefined || options.vercelServiceOutput !== undefined;
  const routeRegistry = createApplicationRouteRegistry(preparedHost, {
    vercelCron: isVercelBuild && preparedHost.scheduleRegistrations.length > 0,
  });
  ...
  const stagedOutputDirectory = workspace.publication.output.stagedDir;
  await measureBuildPhase(profiler, "host.cache.prepare", () =>
    prepareEveVersionedCacheDirectory(stagedOutputDirectory),
  );

  const serverDirectory = isVercelBuild
    ? join(workspace.rootDir, "server-function")
    : join(stagedOutputDirectory, "server");
  const bundle = await measureBuildPhase(profiler, "host.bundle", () =>
    buildApplicationBundle({
      host: preparedHost,
      routeRegistry,
      serverDirectory,
      target: isVercelBuild ? "vercel" : "node",
      ...(routeRegistry.vercelCronRoute === undefined
        ? {}
        : { cronRoute: routeRegistry.vercelCronRoute }),
    }),
  );
  ...
  if (isVercelBuild) {
    await measureBuildPhase(profiler, "vercel.output.emit", () =>
      emitVercelBuildOutput({
        crons: routeRegistry.vercelCrons,
        mode: isServiceOutput ? "service" : "standalone",
        outputDirectory: stagedOutputDirectory,
        routes: routeRegistry.vercelRoutes,
        serverFunctionDirectory: bundle.serverDirectory,
        workflow: {
  ...
  if (vercelServiceOutput !== undefined) {
    await measureBuildPhase(profiler, "vercel.host-middleware.copy", () =>
      copyHostMiddlewareFunctions({
        ...
        serviceOutputDirectory: stagedOutputDirectory,
      }),
    );
  }
  ...
  if (!isVercelBuild) {
    await measureBuildPhase(profiler, "compiler-artifacts.stage", () =>
      stageProductionCompilerArtifacts({
        ...
        outputDir: stagedOutputDirectory,
      }),
    );
  }
  await measureBuildPhase(profiler, "host.cache.write", () =>
    writeEveVersionedCacheMetadata(stagedOutputDirectory),
  );
