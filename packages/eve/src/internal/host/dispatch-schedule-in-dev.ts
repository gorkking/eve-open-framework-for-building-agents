import type { DevelopmentApplicationArtifactsConfig } from "#internal/host/routes/runtime-artifacts.js";
 ...
 * dev-server restart.
 *
 * The artifacts config is resolved before the host bundles this module and
...
export async function dispatchScheduleInDev(input: {
  readonly artifactsConfig: DevelopmentApplicationArtifactsConfig;
  readonly scheduleId: string;
  ...
  const compiledArtifactsSource = resolveApplicationCompiledArtifactsSource(input.artifactsConfig);
  ...
  const { dispatchScheduleTaskFromArtifacts } =
    await import("#internal/host/routes/schedule-task.js");
