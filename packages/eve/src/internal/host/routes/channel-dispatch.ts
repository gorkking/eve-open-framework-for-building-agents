import type { H3Event } from "#compiled/h3/index.js";
...
import { readTrustedDevelopmentClientAddress } from "#internal/host/dev-client-address.js";
...
import { traceChannelRequest } from "#internal/host/routes/channel-request-instrumentation.js";
import { resolveApplicationChannelRuntimeBundle } from "#internal/host/routes/runtime-stack.js";
 ...
 *
 * Each channel route is mounted by the eve host with its route key and
 * artifacts config. H3 matches the URL and populates `event.context.params`,
 * so no custom URL matching is needed — the handler looks up the channel by
 * its `(method, urlPath)` key directly. When routes register background work
 * through `ctx.waitUntil`, the host forwards it to `event.waitUntil()` so
 * webhook acknowledgements can return immediately.
 *
...
export async function dispatchChannelRequest(
  ...
  return await traceChannelRequest({ request: event.req, routeKey }, async (span) => {
    const bundle = await resolveApplicationChannelRuntimeBundle(config);
    ...
    try {
      ...
      // Without this a handler throw is only the host's default 5xx, with no eve
...
function buildRouteArgs(
  event: H3Event,
  bundle: Awaited<ReturnType<typeof resolveApplicationChannelRuntimeBundle>>,
  channelName: string,
  channelKind: string,
  config: ApplicationArtifactsConfig,
  ...
  const args = attachRouteSessionCreator(
    attachAgentInfoRouteResponse(
      ...
      async () => {
        const { handleAgentInfoRequest } = await import("#internal/host/routes/info.js");
...
function extractRequestIp(event: H3Event, config: ApplicationArtifactsConfig): string | null {
  if (config.kind === "development") {
