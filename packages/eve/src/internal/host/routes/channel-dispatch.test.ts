...
vi.mock("#internal/host/routes/runtime-stack.js", () => ({
  resolveApplicationChannelRuntimeBundle: vi.fn(),
}));
...
const mockedResolveApplicationChannelRuntimeBundle = vi.mocked(
  resolveApplicationChannelRuntimeBundle,
);
...
describe("dispatchChannelRequest", () => {
  it("supplies the current linked project to Vercel OIDC during local development", async () => {
    ...
    mockedResolveApplicationChannelRuntimeBundle.mockResolvedValue({
      channels: [
        {
  ...
  it("returns the response before background work settles when the host provides waitUntil", async () => {
...
describe("dispatchChannelRequest tracing", () => {
  ...
  it("records the exception and rethrows when runtime-bundle resolution fails", async () => {
    mockedResolveApplicationChannelRuntimeBundle.mockRejectedValue(new Error("bundle boom"));
  ...
  it("still emits a 404 span with the route but no channel identity when nothing matches", async () => {
    mockedResolveApplicationChannelRuntimeBundle.mockResolvedValue({ channels: [], runtime });
