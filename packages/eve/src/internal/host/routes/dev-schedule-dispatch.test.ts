...
vi.mock("#internal/host/dispatch-schedule-in-dev.js", async () => {
  const actual = await vi.importActual<typeof import("#internal/host/dispatch-schedule-in-dev.js")>(
    "#internal/host/dispatch-schedule-in-dev.js",
  );
  return {
    ...actual,
...
async function importHandler() {
  return await import("#internal/host/routes/dev-schedule-dispatch.js");
}
...
describe("handleDevScheduleDispatchRequest", () => {
  ...
  it("returns 404 with the list of available schedule ids when the schedule is unknown", async () => {
    const { UnknownDevScheduleError } = await import("#internal/host/dispatch-schedule-in-dev.js");
  ...
  it("propagates unexpected errors so the eve host can render a 500", async () => {
