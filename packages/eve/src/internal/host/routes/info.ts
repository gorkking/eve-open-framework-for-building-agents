...
import { buildAgentInfoResponseFromManifest } from "#internal/host/routes/agent-info/build-agent-info-response-from-manifest.js";
import {
  loadAgentInfoManifestData,
  resolveAgentInfoCompiledArtifactsSource,
} from "#internal/host/routes/agent-info/load-agent-info-data.js";
...
async function createAgentInfoPayload(input: ApplicationArtifactsConfig) {
 ...
 */
export async function handleAgentInfoRequest(input: ApplicationArtifactsConfig): Promise<Response> {
