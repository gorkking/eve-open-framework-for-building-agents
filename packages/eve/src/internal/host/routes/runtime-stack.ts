...
import {
  type ApplicationArtifactsConfig,
  resolveApplicationCompiledArtifactsSource,
} from "#internal/host/routes/runtime-artifacts.js";

/**
 * Bundle returned to the per-channel eve host dispatch handler.
 *
 ...
 */
export interface ApplicationChannelRuntimeBundle {
 ...
 */
export async function resolveApplicationChannelRuntimeBundle(
  config: ApplicationArtifactsConfig,
): Promise<ApplicationChannelRuntimeBundle> {
  const compiledArtifactsSource = resolveApplicationCompiledArtifactsSource(config);
