...
} from "#internal/host/prepare-application-host.js";
...
describe("host preparation", () => {
  ...
  it("keeps production compiler and host writes inside one invocation workspace", async () => {
    ...
    try {
      expect(workspace.host.artifactsDir).toBe(join(workspace.rootDir, "host"));
      expect(workspace.host.buildDir).toBe(join(workspace.rootDir, "host-build"));
  ...
  it("keeps host inputs outside retained runtime generations", async () => {
