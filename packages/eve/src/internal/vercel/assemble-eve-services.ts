...
export function assembleEveVercelServices(input: {
  ...
  for (const { agent, target } of input.agents) {
    ...
    const serviceName = configured?.name ?? createEveServiceName(agent.name);
    ...
    if (configured === undefined) {
      ...
      rootDirectories.push(contribution.rootDirectory);
    } else {
      services[serviceName] = {
        ...configured.service,
        routes: insertEveServiceRequestPathRoute(configured.service.routes, routeSrc),
      };
    }

    eveRoutes.push({ routeSrc, serviceName });
  }
