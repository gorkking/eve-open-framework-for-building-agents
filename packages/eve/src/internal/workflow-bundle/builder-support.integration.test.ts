...
describe("bundleFinalWorkflowOutput", () => {
  it("writes a base64 wrapper against the namespaced eve workflow runtime facade", async () => {
  ...
  it("preserves inline source maps in executable step registrations", async () => {
    ...
    try {
      ...
      expect(source).toContain("sourceMappingURL=data:");
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
