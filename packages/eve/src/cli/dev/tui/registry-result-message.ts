import type { RegistrySessionItemFailure, RegistrySessionItemResult } from "#setup/flows/registry-session.js";
...
export function formatRegistrySessionResult(result: RegistryResultReport): string {
  const lines: string[] = [];
  if (result.items.length > 0) lines.push(`Added ${joinedTitles(result.items.map((item) => item.title))}`);
