 ...
 * `input.requested` (posts the prompt and options), `message.completed` (posts
 * the reply), and `session.failed`/`turn.failed` (posts an error comment). A
 * handler supplied here replaces the built-in for that key rather than running
 * alongside it.
 */
