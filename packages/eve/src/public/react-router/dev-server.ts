...
const EVE_REACT_ROUTER_DEV_SERVER_FILE_NAME = "react-router-dev-server.json";
const EVE_REACT_ROUTER_DEV_SERVER_LOCK_FILE_NAME = "react-router-dev-server.lock";
...
function resolveEveDevServerRegistryPath(appRoot: string): string {
  return join(resolveEveCacheDirectory(appRoot), EVE_REACT_ROUTER_DEV_SERVER_FILE_NAME);
}

function resolveEveDevServerLockPath(appRoot: string): string {
  return join(resolveEveCacheDirectory(appRoot), EVE_REACT_ROUTER_DEV_SERVER_LOCK_FILE_NAME);
}
  ...
  while (true) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          `Timed out after ${DEV_SERVER_REGISTRY_TIMEOUT_MS}ms waiting for another React Router process to start eve.`,
        );
      }
 ...
 * cross-process lock so concurrent React Router processes don't each boot eve.
 */
