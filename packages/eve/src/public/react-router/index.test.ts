...
import { eveReactRouter } from "./index.js";
...
describe("eveReactRouter", () => {
  ...
  it("validates vercel.json services during Vercel production builds", async () => {
    vi.stubEnv("VERCEL", "1");
    const plugin = eveReactRouter({ eveBuildCommand: "pnpm build:eve", eveRoot: "agent" });

    const result = await getConfigHook(plugin)({}, { command: "build", mode: "production" });
  ...
  it("fails Vercel production builds that need a generated service with the vercel.json to add", async () => {
    vi.stubEnv("VERCEL", "1");
    ensureEveVercelServicesConfigMock.mockResolvedValueOnce({
      mode: "generated",
      services: {
        eve: {
    ...
    await expect(
      getConfigHook(plugin)({}, { command: "build", mode: "production" }),
    ).rejects.toThrow(/Declare the services in vercel\.json/);
  });

  it("does not spawn an eve dev server for serve-mode config loads during Vercel builds", async () => {
    vi.stubEnv("VERCEL", "1");
    const plugin = eveReactRouter();

    const result = await getConfigHook(plugin)({}, { command: "serve", mode: "development" });
