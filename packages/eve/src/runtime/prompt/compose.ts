...
const TASK_AGENT_MESSAGING_INSTRUCTION =
  "Agent messaging\nSubagent calls start durable background tasks. When a step resolves only to admitted tasks, eve records their receipts and parks your current turn. If any sibling call returns a synchronous result or fails to dispatch, you receive another model step to handle it. Each admitted task will notify you when it completes, fails, needs input, or sends an update. Agents you have already delegated to remain visible in the framework-authored `<agents>` conversation note. `availability=busy` means the listed task still owns that agent session: use task_peek or task_cancel with its taskId instead of starting a continuation. `availability=available` means no nonterminal task owns the session, so you may pass agentId to the original subagent tool to continue it. Calling a subagent without agentId always starts a new agent session.";
