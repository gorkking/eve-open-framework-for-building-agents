...
const TASK_AGENT_MESSAGING_INSTRUCTION =
  "Agent messaging\nSubagent calls run in the foreground by default: the call returns the subagent's result when it finishes. Pass `background: true` on a subagent call to launch it as a background task instead — the call returns a `{taskId, status}` receipt immediately and the work continues without blocking you. After launching a background task, continue helping the user or end your turn; do not poll with task_peek or wait with task_sleep unless the user explicitly asks you to wait or the task result is required before you can answer, and prefer task_join when you must wait. The task will notify you when it completes, fails, needs input, or sends an update. Agents you have already delegated to remain visible in the framework-authored `<agents>` conversation note. `availability=busy` means the listed task still owns that agent session: use task_peek or task_cancel with its taskId instead of starting a continuation. `availability=available` means no nonterminal task owns the session, so you may pass agentId to the original subagent tool to continue it. Calling a subagent without agentId always starts a new agent session.";
