...
describe("subagent tool input schema variants", () => {
  ...
  it("derives the variant from the agent opt-in state", () => {
    ...
    expect(resolveSubagentToolSchemaVariant({ persistentSessions: true })).toBe("persistent");
    expect(resolveSubagentToolSchemaVariant({ tasks: true })).toBe("tasks");
    expect(resolveSubagentToolSchemaVariant({ persistentSessions: true, tasks: true })).toBe(
      "tasks",
    );
  });
});
