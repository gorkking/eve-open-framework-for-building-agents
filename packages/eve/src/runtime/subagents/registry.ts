...
export function resolveSubagentToolSchemaVariant(input?: {
  ...
  return input?.subagentPersistentSessions === true ? "persistent" : "plain";
}
...
export function createRuntimeSubagentRegistry(input: {
  ...
  const inputSchema = getSubagentToolInputJsonSchema(
    resolveSubagentToolSchemaVariant({
      subagentPersistentSessions: input.persistentSessions,
      tasks: input.tasks,
    }),
  );
