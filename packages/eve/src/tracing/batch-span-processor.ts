...
export function batchSpanProcessor(
  ...
  return {
    async forceFlush() {
      ...
      pending = pending.then(() => settleExporter("exporter flush", exporter.forceFlush!));
      await waitForExporter(pending, "exporter flush");
    },
    onEnd(span) {
      if (stopped) return;
    ...
    async shutdown() {
      ...
      buffer = [];
      if (!drained) return;
      pending = pending.then(() => settleExporter("exporter shutdown", exporter.shutdown));
      await waitForExporter(pending, "exporter shutdown");
    },
  };
