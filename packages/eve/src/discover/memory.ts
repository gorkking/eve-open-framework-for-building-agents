...
/** Discovers the XOR flat-file or named-directory memory authoring form. */
export async function discoverMemorySources(input: {
  ...
  const directory = hasDirectory
    ? await discoverNamedSourceDirectory({
  ...
  if (flat.module !== undefined && hasDirectory) {
    return {
      diagnostics: [
        ...flat.diagnostics,
  ...
  return {
    ...
    memories: directory.sources.map((source) => ({
      ...source,
      slot: source.logicalPath.slice("memory/".length).replace(/\.[^.]+$/u, ""),
    })),
  };
}
