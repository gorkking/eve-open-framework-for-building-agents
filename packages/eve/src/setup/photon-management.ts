...
/** Registers the webhook URL and returns its one-time signing secret. */
export async function registerPhotonWebhook(input: {
  projectId: string;
  projectSecret: string;
  webhookUrl: string;
  deps?: Pick<PhotonManagementDeps, "fetch">;
}): Promise<string> {
  const request = input.deps?.fetch ?? fetch;
  const response = await request(
    `${PHOTON_SPECTRUM_HOST}/projects/${encodeURIComponent(input.projectId)}/webhooks/`,
    {
      method: "POST",
  ...
  const body = await json(response, "webhook registration");
  const registration = record(body["data"] ?? body);
  if (typeof registration["signingSecret"] !== "string") {
    throw new Error("Photon did not return the webhook signing secret.");
  }
