...
describe("Photon management provisioning", () => {
  ...
  test("registers the Connect URL and returns the one-time secret", async () => {
    const fetch = vi.fn<typeof globalThis.fetch>(async () =>
      response({
        succeed: true,
        data: { id: "webhook-id", signingSecret: "webhook-secret" },
      }),
    );
    ...
    await expect(
      registerPhotonWebhook({
        projectId: "project-id",
        projectSecret: "project-secret",
        webhookUrl: "https://connect.vercel.com/trigger/scl_123",
        deps: { fetch },
      }),
    ).resolves.toBe("webhook-secret");

    expect(fetch).toHaveBeenCalledWith(
      "https://spectrum.photon.codes/projects/project-id/webhooks/",
