...
async function chooseSetupPlan(
  ...
  try {
    ...
    if (context.presetPortableCredentials !== undefined) {
      ...
      throw new Error(
        "iMessage setup needs the public HTTPS URL that will receive Photon webhooks. Run `eve add channel/photon` interactively.",
      );
    }
...
export const PHOTON_CHANNEL_SETUP: ChannelSetupIntegration = {
  kind: "photon",
  label: "Photon",
  ...
  async setup(context) {
    ...
    const result = await runChannelSetup(
      ...
      plan.credentials === "vercel-connect"
        ...
        : {
            ...
            photonWebhookBaseUrl: plan.webhookBaseUrl,
            photonProject: plan.photonProject,
            photonProjectName: plan.photonProjectName,
          },
    );
    if (
      ...
      result.state.channels.includes("photon")
    ) {
