...
import type { RuntimeSubagentChildResult, RuntimeSubagentResult } from "#runtime/actions/types.js";
...
export async function notifyTurnCallerStep(input: {
  ...
  if (input.caller.replyTo.kind === "callback") {
    await postSettledTurnCallback({ result, url: input.caller.replyTo.url });
    return;
  }
...
function createSettledTurnResult(input: {
  ...
  if (input.settled.isError === true) {
    ...
    return {
      ...
      output: error,
      // The child's own session id: the parent verifies it against the
      // identity captured at dispatch before the result may settle the call.
      sessionId: input.sessionId,
