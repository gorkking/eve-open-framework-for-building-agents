...
export function buildRunContext(input: {
  ...
  if (run.progressCallback !== undefined) {
    ctx.set(ProgressCallbackKey, run.progressCallback);
  }
  if (run.progressWork !== undefined) {
    ctx.set(ProgressLineageKey, run.progressWork);
  }
