...
describe("task_join over a real background task", () => {
  it("leaves a working join pending, settles it from the poll step, then settles immediately once terminal", async () => {
    ...
    await runtime.run(async () => {
      ...
      const dispatched = await dispatchTaskStep({
        ...
        serializedContext: {
          ...
          "eve.sessionId": session.sessionId,
        },
        sessionState: createDurableSessionState({ session }),
      });
      const pendingTask = dispatched.pendingTasks[0];
      const handle = getAgentHandleStore(dispatched.sessionState.snapshot?.session.state)
        ?.handles[0];
      ...
      const taskSession = dispatched.sessionState.snapshot?.session as RuntimeSession | undefined;
