...
interface BundleReportModule {
  collectBundleReport(input: {
    appLabel: string;
    appRoot: string;
    packageLabel?: string;
    packageRoot?: string;
  ...
  compareBundleReports(
    report: BundleReport,
    baselineReport: BundleReport,
    options?: {
      baselineLabel?: string;
    },
  ): BundleReport["comparison"];
  renderBundleReportMarkdown(report: BundleReport): string;
}
...
async function loadBundleReportModule(): Promise<BundleReportModule> {
  const moduleUrl = new URL("../../../../scripts/bundle-report.mjs", import.meta.url);

  return (await import(moduleUrl.href)) as BundleReportModule;
}
...
async function createReportFixture(
  ...
  const appRoot = await createScratchDirectory("eve-bundle-report-");
...
describe("bundle report", () => {
  it("groups symlinked Vercel functions by their real payload and includes publish metrics", async () => {
    ...
    const { collectBundleReport, renderBundleReportMarkdown } = await loadBundleReportModule();
    ...
    const markdown = renderBundleReportMarkdown(report);

    expect(report.uniqueFunctionCount).toBe(1);
    expect(report.buildMetadata).toEqual({
      frameworkName: "eve",
    ...
    expect(markdown).toContain("Vercel Build Output API v3");
    expect(markdown).toContain("Framework: `eve@0.0.1`");
    expect(markdown).not.toContain("Nitro");
  ...
  it("renders metric deltas against a main baseline snapshot", async () => {
    ...
    const { collectBundleReport, compareBundleReports, renderBundleReportMarkdown } =
      await loadBundleReportModule();
    const baselineReport = await collectBundleReport({
      appLabel: "apps/fixtures/weather-agent",
      appRoot: baselineFixture.appRoot,
      packageLabel: "packages/eve",
      packageRoot: baselineFixture.packageRoot,
    });
    const collectedCurrentReport = await collectBundleReport({
      appLabel: "apps/fixtures/weather-agent",
      appRoot: currentFixture.appRoot,
      packageLabel: "packages/eve",
      packageRoot: currentFixture.packageRoot,
    });
    const currentReport: BundleReport = {
    ...
    const comparison = compareBundleReports(currentReport, baselineReport, {
      baselineLabel: "main",
    });
    ...
    const acknowledgedMarkdown = renderBundleReportMarkdown({
      ...currentReport,
      comparison,
  ...
  it("omits repetitive takeaways when the comparison has no notable delta", async () => {
    ...
    const comparison = compareBundleReports(report, report, {
      baselineLabel: "main",
    });
    const markdown = renderBundleReportMarkdown({
      ...report,
